...
namespace Droid_Image
{
	...
	public class Interface_image : GPInterface
	{
        ...
        private void LaunchGoogleImg(string direction = "")
        {
            ...
            {
                _webSearchUrls = GoogleImage.GetUrls(GoogleImage.GetHtmlCode(_textSearch));
                _indexImageWeb = -1;
            }
            ...
            for (int i = 0; i < 42; i++)
            {
                ...
                pic = GoogleImage.GetImage(luckyUrl);
                if (pic != null)
                {
                    _pictureByte = pic;
                    break;
            ...
            if (_pictureByte != null)
            {
                if (_stream != null) { _stream.Close(); }
                _stream = new MemoryStream(_pictureByte);
                CurrentImage = Bitmap.FromStream(_stream);
            }
            else
            {
                if (_stream != null) { _stream.Close(); }
