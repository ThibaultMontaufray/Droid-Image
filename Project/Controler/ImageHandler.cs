...
namespace Droid_Image
{
    public class ImageHandler
    {
        ...
        public void SetGrayscale()
        {
            try
            {
                Bitmap d = new Bitmap(_currentBitmap.Width, _currentBitmap.Height);

                for (int i = 0; i < _currentBitmap.Width; i++)
                {
                    for (int x = 0; x < _currentBitmap.Height; x++)
                    {
                        Color oc = _currentBitmap.GetPixel(i, x);
                        int grayScale = (int)((oc.R * 0.3) + (oc.G * 0.59) + (oc.B * 0.11));
                        Color nc = Color.FromArgb(oc.A, grayScale, grayScale, grayScale);
                        d.SetPixel(i, x, nc);
                    }
                }
                _currentBitmap = (Bitmap)d.Clone();
            }
            catch (Exception)
            {
        ...
        public void Crop(int xPosition, int yPosition, int width, int height)
        {
            ...
            Bitmap temp = (Bitmap)_currentBitmap;
            Bitmap bmap = (Bitmap)temp.Clone();
            if (xPosition + width > _currentBitmap.Width) width = _currentBitmap.Width - xPosition;
            if (yPosition + height > _currentBitmap.Height) height = _currentBitmap.Height - yPosition;
        ...
        public void DrawOutCropArea(int xPosition, int yPosition, int width, int height)
        {
            ...
            _bitmapPrevCropArea = (Bitmap)_currentBitmap.Clone();
            Bitmap workingBM = (Bitmap)_currentBitmap.Clone();
            //Graphics gr = Graphics.FromImage(_currentBitmap);
            ...
            //for (int i = 1; i < _currentBitmap.Width; i++)
            //{
            //    for (int j = 1; j < yPosition; j++)
            //    {
            //        workingBM.SetPixel(i, j, themeColor);
            //    }
            //    for (int j = (yPosition + height); j < _currentBitmap.Height; j++)
            //    {
            //        workingBM.SetPixel(i, j, themeColor);
            //    }
            //}
            ...
            //{
            //    for (int j = yPosition; j < height; j++)
            //    {
            //        workingBM.SetPixel(i, j, themeColor);
            //    }
            //}

            //for (int i = (xPosition + width); i < (_currentBitmap.Width - xPosition - width); i++)
            //{
            //    for (int j = yPosition; j < height; j++)
            //    {
            //        workingBM.SetPixel(i, j, themeColor);
